...
const cloudinaryMultiUpload = async (req: Request, res: Response, next: NextFunction) => {
  const fileReq = req as Request & { files?: MulterFile[] | { [fieldname: string]: MulterFile[] } };
  
  // Check if files exist
  if (!fileReq.files || (Array.isArray(fileReq.files) && fileReq.files.length === 0)) {
